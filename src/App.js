			...
			<main className="text-white">
				<Container className="py-16">
					...
					<div className="flex justify-between mb-4 items-center">
						...
						<Button onClick={sendRequest} disabled={loading}>Submit</Button>
					</div>
					<div class="w-full">
						<div class="animate-pulse flex"> 
							<div class="flex-1">
							<div class={`h-1 ${loading ? 'bg-green-700' : 'bg-dark'} transition`}></div>
							</div>
						</div>
					</div>
					<AceEditor
					...
					{data && data.judged && 
						<div className={` ${data.status.id === 0 ?'bg-gray-700' : (data.status.id === 3 ? 'bg-green-700' : 'bg-red-800')} p-4 fixed z-50 bottom-0 left-0 right-0`}>
